...
Add_FE  <- function( dataset )
 {
  ...
  dataset[, dist_msaldo_mpayroll := mcuentas_saldo - mpayroll] #si
  
  # mcuentas_saldo - mcaja_ahorro
  dataset[, dist_msaldo_mcaja := mcuentas_saldo - mcaja_ahorro] #si
  ...
  dataset[, dist_mcaja_mcorriente := mcaja_ahorro - mcuenta_corriente] #si
  

    #PRESTAMOS: Ratios y Distancias
  # mprestamos_personales / cprestamos_personales
  dataset[, mprestamo_avg := ifelse(cprestamos_personales == 0, 100, mprestamos_personales / cprestamos_personales)] #si
  ...
  dataset[, prestamos_antiguedad_ratio := ifelse(cliente_antiguedad <= 0, 100, mprestamos_personales / cliente_antiguedad)] #si
  
  #RENTABILIDAD: Ratios y Distancias
  # mrentabilidad - mcomisiones_mantenimiento
  dataset[, dist_mrentabilidad_mcomisiones := mrentabilidad - mcomisiones_mantenimiento]  #si
  
  # mrentabilidad / mcomisiones_mantenimiento
  dataset[, rentabilidad_comisiones_ratio := ifelse(mcomisiones_mantenimiento == 0, 100, mrentabilidad / mcomisiones_mantenimiento)] #si
  ...
  # mrentabilidad / mpasivos_margen
  dataset[, rentabilidad_pasivos_ratio := ifelse(mpasivos_margen <= 0, 100, mrentabilidad / mpasivos_margen)] #si
  
  # mrentabilidad / ctrx_quarter_fix
  dataset[, rentabilidad_ctrx_ratio := ifelse(ctrx_quarter_fix == 0, 100, mrentabilidad / ctrx_quarter_fix)] #si
  ...
  dataset[, rentannual_rent_ratio := ifelse(mrentabilidad == 0, 100, mrentabilidad_annual / mrentabilidad)] #si30
  
  # mrentabilidad_annual / mcomisiones_mantenimiento
  dataset[, rentannual_comisiones_ratio := ifelse(mcomisiones_mantenimiento == 0, 100, mrentabilidad_annual / mcomisiones_mantenimiento)] #si
  
  
  # TARJETAS:
  # Cantidad de tarjetas total sum(ctarjeta_visa, ctarjeta_master)
  dataset[, ctarjetas := rowSums(.SD, na.rm = TRUE), .SDcols = c("ctarjeta_visa", "ctarjeta_master")] #si
  ...
  dataset[, ratio_vm_msaldototal_payroll := ifelse(mpayroll <= 0, 100, (Visa_msaldototal + Master_msaldototal) / mpayroll)] #si
  ...
  dataset[, ratio_vm_msaldopesos_payroll := ifelse(mpayroll <= 0, 100, vm_msaldopesos / mpayroll)] #si
  ...
  dataset[, ratio_vm_msaldopesos_limites := ifelse((Visa_mlimitecompra + Master_mlimitecompra) == 0, 100, vm_msaldopesos / (Visa_mlimitecompra + Master_mlimitecompra))]
  
  # sum(Visa_mpagominimo,Master_mpagominimo)
  dataset[, vm_mpagominimo := Visa_mpagominimo + Master_mpagominimo] #si
  # sum(Visa_mpagominimo,Master_mpagominimo) / mpayroll
  dataset[, ratio_vm_mpagominimo_payroll := ifelse(mpayroll <= 0, 100, vm_mpagominimo / mpayroll)] #si 7
  ...
  dataset[, ratio_vmtarjeta_consumos_tarjetas := ifelse(ctarjetas == 0, 100, vmtarjeta_consumos / ctarjetas)] #si
  ...
  dataset[, ratio_vmtarjeta_consumos_limites := ifelse((Visa_mlimitecompra + Master_mlimitecompra) == 0, 100, vmtarjeta_consumos / (Visa_mlimitecompra + Master_mlimitecompra))]
  ...
  dataset[, msaldovs_mprest := (Visa_msaldototal + Master_msaldototal) + mprestamos_personales]
...
escalar_variable  <- function( campos_drift )
{
  ...
  {
    cat( campo, " " )
    dataset[, paste0("scale_",campo) := scale(get(campo),center = TRUE, scale = TRUE)]
    dataset[ , (campo) := NULL ]
  }
} 
...
#cargo el dataset
dataset <- fread("./datasets/competencia03_fix.csv.gz") 
...
#Agregar el Feature Engineering (FE): Agrega 75 variables -> 159 + 55 = 214
Add_FE (dataset)

dim(dataset) #4562810     214
...
#EXPORTAMOS a un .CSV
fwrite( dataset,
        file= "competencia03_FE_drift.csv.gz",
        sep= "," )
